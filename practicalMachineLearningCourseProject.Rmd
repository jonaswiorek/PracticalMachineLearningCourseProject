...
The data was available to download from Groupware website.
...
methods to remove covariates reduce the number of covariates used for the model to `r  length(trainingXNaNzv) -1`.

Prediciting with trees generates poor accuracy. A more advanced model was therefore required. A random forest model was selected. 

```{r}
modelFitXNaNzvRf <- train(classe ~ ., method = "rf", data = trainingXNaNzv)
```
The validation set is used to cross-validate the model. 
...
```
The cross-validation shows the random forest model gives an overall accuracy of `r confusionMatrixXNaNzvRf$overall[[1]]`%. 
